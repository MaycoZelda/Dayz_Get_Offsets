...
protected:
	...
	INT64 ResolveMovRegXmm(UINT64 Module, UINT64 Instruction);
	INT64 ResolveMovRegXmmLrg(UINT64 Module, UINT64 Instruction);
	INT64 ResolveMovRegXmmByte(UINT64 Module, UINT64 Instruction);
	INT64 ResolveMovRegXmmLrgByte(UINT64 Module, UINT64 Instruction);
	...
	INT64 ResolveMovRegByteSml(UINT64 Module, UINT64 Instruction);
...
private:
	bool SetupPatterns();
	void SaveOffsetsToFile();
	void SaveDebugReport();
	// Tenta seguir ponteiros dinâmicos para resolver offsets multinível
	bool TryResolvePointerChain(PBYTE Module, INT64 PointerOffset, INT64& OutPointerValue);
