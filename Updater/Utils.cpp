...
UINT64 Utils::PatternScan(UINT64 Module, PBYTE Allocated, const char* Section, PBYTE Pattern, const char* Mask) {
	...
	// Debug: imprimir quantas seções existem
	printf("[PATTERNSCAN] Procurando padrão na seção '%s' (%d seções disponíveis)\n", 
		   Section, Nt->FileHeader.NumberOfSections);

	bool SectionFound = false;

	for (auto i = 0; i < Nt->FileHeader.NumberOfSections; i++, Info++) {
		// Check if this section matches the requested section name
		printf("[PATTERNSCAN] Seção disponível: %s\n", Info->Name);

		if (strcmp((const char*)Info->Name, Section) != 0)
			continue;

		SectionFound = true;

		// Scan the entire section from VirtualAddress to VirtualAddress + SizeOfRawData
		UINT64 SectionStart = Info->VirtualAddress;
		UINT64 SectionEnd = SectionStart + Info->SizeOfRawData;

		printf("[PATTERNSCAN] ✅ Seção encontrada! Iniciando busca de 0x%llx a 0x%llx\n", 
			   SectionStart, SectionEnd);

		for (UINT64 j = SectionStart; j < SectionEnd; j++) {
			if (DataCompare(Allocated + j, Pattern, Mask)) {
				printf("[PATTERNSCAN] ✅ Padrão encontrado em 0x%llx\n", Module + j);
				return Module + j;
			}
		}

		printf("[PATTERNSCAN] ❌ Padrão NÃO encontrado nesta seção\n");
	}

	if (!SectionFound) {
		printf("[PATTERNSCAN] ❌ Seção '%s' não existente! Seções disponíveis acima.\n", Section);
	}
